...
import {
  ...
  Lightbulb, BookMarked, Newspaper, RefreshCw, Loader2, Trash2, Archive, CheckCircle2, X, Wand2,
} from "lucide-react";
import {
  listPages, getDiaryStats, signDiaryPaths, setPageStatus, deletePages, listAssets,
  type DiaryPage, type DiaryStats, type DiaryStatus, type DiaryAsset, type DiaryAssetKind,
} from "@/lib/diary";
...
export default function DiaryAdmin() {
  ...
  const [assets, setAssets] = useState<DiaryAsset[]>([]);
  const [showGenerate, setShowGenerate] = useState(false);
  const [assetView, setAssetView] = useState<DiaryAsset | null>(null);
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 rounded-xl bg-primary/5 border border-primary/20">
              ...
              <div className="flex flex-wrap gap-2 ml-auto">
                <button
                  ...
                  disabled={bulkBusy}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 disabled:opacity-50 transition"
                >
      ...
      {tab === "assets" && (
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-border bg-card/40 p-12 text-center">
              <Sparkles className="w-10 h-10 text-muted-foreground/25 mx-auto mb-3" strokeWidth={1.5} />
              <p className="text-sm font-medium mb-1">Nothing generated yet</p>
              <p className="text-xs text-muted-foreground mb-5 max-w-md mx-auto">
                Go to the Library, tick the diary pages you want to draw from, then choose{" "}
                <strong>Generate…</strong> to turn them into a PDF, audio script, template, insight
                report, research paper or article.
              </p>
              <button
              ...
              >
                <Search className="w-3.5 h-3.5" /> Go to Library
              </button>
            </div>
          ) : (
            <>
              <p className="text-xs text-muted-foreground">
                {assets.length} generated item{assets.length === 1 ? "" : "s"}
              </p>
              <div className="space-y-2">
                {assets.map((a) => {
                  const Icon = ASSET_ICON[a.kind] ?? FileText;
                  return (
                    <button
                      key={a.id}
                      onClick={() => setAssetView(a)}
                      className="w-full flex items-center gap-3 p-3.5 rounded-2xl border border-border bg-card/60 hover:border-primary/40 text-left transition-colors"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          ...
                          {" · "}
                          {a.source_page_ids.length} source page
                          {a.source_page_ids.length === 1 ? "" : "s"}
                          {" · "}
